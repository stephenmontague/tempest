package app.tempest.temporal.config;
 ...
 *
 * Lives in the tempest-temporal library, which only the worker apps depend on — so the
 * REST API app never gets a Temporal connection (no role flag needed).
 */
@Configuration
