...
export async function GET(request: Request, { params }: RouteParams) {
     ...
     if (isNaN(shipmentId)) {
          return NextResponse.json({ error: "Bad Request", message: "Invalid shipment ID" }, { status: 400 });
     }

     try {
          const smsClient = getSmsClient();
          const shipment = await smsClient.getShipment(shipmentId, session.accessToken);
