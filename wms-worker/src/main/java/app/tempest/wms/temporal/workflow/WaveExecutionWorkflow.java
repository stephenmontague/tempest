...
public interface WaveExecutionWorkflow {
     ...
     /**
      * Select a rate for a shipment (request-response via Update).
      *
      * This is a synchronous, validated operation: the validator rejects the request if
      * the shipment is unknown or not in CREATED state; the handler persists the rate and
      * returns the updated shipment state. (Rate shopping is optional — callers can skip it
      * and use the default carrier.)
      *
      * @param shipmentId   The shipment ID
      * @param carrier      The selected carrier
      * @param serviceLevel The selected service level
      * @return the updated shipment state
      */
     @UpdateMethod
     ShipmentStateDTO rateSelected(Long shipmentId, String carrier, String serviceLevel);

     @UpdateValidatorMethod(updateName = "rateSelected")
     void validateRateSelected(Long shipmentId, String carrier, String serviceLevel);
