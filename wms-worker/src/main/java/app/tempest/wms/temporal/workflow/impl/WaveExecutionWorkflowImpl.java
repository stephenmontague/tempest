...
public class WaveExecutionWorkflowImpl implements WaveExecutionWorkflow {
     ...
     private void confirmShipment(Long shipmentId) {
          ...
          ConfirmShipmentRequest confirmRequest = ConfirmShipmentRequest.builder()
                    ...
                    .shipmentId(shipmentId)
                    .orderId(shipment.getOrderId())
                    // Deterministic workflow time (not Instant.now()) so this replays correctly.
                    .shippedAt(Instant.ofEpochMilli(Workflow.currentTimeMillis()))
                    .build();
     ...
     /**
      * Update validator for rateSelected: read-only, rejects invalid requests before they
      * are admitted to history. Must not mutate state or block (no activities/timers).
      */
     @Override
     public void validateRateSelected(Long shipmentId, String carrier, String serviceLevel) {
          if (carrier == null || carrier.isBlank() || serviceLevel == null || serviceLevel.isBlank()) {
               throw new IllegalArgumentException("carrier and serviceLevel are required");
          }
          ShipmentStateDTO shipment = shipmentStates.get(shipmentId);
          if (shipment == null) {
               throw new IllegalArgumentException("Unknown shipmentId: " + shipmentId);
          }
          if (!"CREATED".equals(shipment.getStatus())) {
               throw new IllegalStateException(
                         "Rate can only be selected while shipment is CREATED (was " + shipment.getStatus() + ")");
          }
     }

     @Override
     public ShipmentStateDTO rateSelected(Long shipmentId, String carrier, String serviceLevel) {
          ShipmentStateDTO shipment = shipmentStates.get(shipmentId);

          // Persist the selected rate in SMS, then update local state.
          ...
          // Return the updated state to the caller (request-response).
          return shipment;
     }
